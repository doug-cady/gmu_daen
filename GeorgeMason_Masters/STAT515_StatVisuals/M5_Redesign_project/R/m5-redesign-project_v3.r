...
merged_incomes <- left_join(incomes_no_us, fmt_top_incomes) %>%
    select(state, top_bot_ratio, mean_ratio, "1928", "1973", "2015") %>%
    rename(inc_share_1928 = "1928",
           inc_share_1973 = "1973",
           inc_share_2015 = "2015")

# glimpse(merged_incomes)
# summary(merged_incomes)


pan_desc <- data.frame(
    type = c("mapcum", "id", "dot", "arrow", "arrow"),

    lab1 = c(NA, NA, "2015 Mean Annual Income", "Top 1% Share of All Income",
             "Top 1% Share of All Income"),
    lab2 = c(NA, NA, "Ratio Top 1% to Bot 99%", "1928 - 1973",
             "1973 - 2015"),

    col1 = c(NA, NA, "top_bot_ratio", "inc_share_1928", "inc_share_1973"),
    col2 = c(NA, NA, NA, "inc_share_1973", "inc_share_2015"),
...
pdf(file = "../graphics/Income_Inequality_Redesign_V3.pdf",
    width = 8.5, height = 11)
...
micromapST(merged_incomes, pan_desc,
           ...
           title = c("2015 US State Income Inequality - Top 1% vs Bottom 99%",
                     "and Top 1% Share of All Income over time")
)
dev.off()
