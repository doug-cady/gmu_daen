...
## Juxtaposed row-labeled plots - population size vs freedom score
...
grid.arrange(gg_pop, gg_freedom, ncol = 2, top = textGrob(grid_title))

pop_freedom_juxta_plot <- arrangeGrob(gg_pop, gg_freedom, ncol = 2, top = textGrob(grid_title))
ggsave(plot = pop_freedom_juxta_plot, "../graphics/juxta_row_pop_freedom.png",
       width = 7, height = 9, units = "in")
```


## Juxtaposed row-labeled plots - freedom score vs suicide rate
```{r freedom_vs_suicides_plot, fig.height = 7, fig.width = 10, warning = FALSE, message = FALSE}
# TODO: INCLUDE TOP 51
free_suic_juxta_data <- arrange(juxta_data, -hf_score, total_suicides_p100k)
free_suic_juxta_data$prox_grp <- rep(paste0("G", 1:5), each = 5)
free_suic_juxta_data$color_grp <- rep(c("CG1", "CG2", "CG3", "CG4", "CG5"), times = 5)


# Left side of freedom vs suicides
gg_freedom_2 <- free_suic_juxta_data %>%
    ggplot(aes(x = hf_score, y = reorder(country, hf_score),
               group = prox_grp, color = color_grp)) +
    geom_path(color = "gray60") +
    geom_point(size = 3.5) +
    geom_point(color = "white", size = 1) +
    facet_grid(prox_grp ~ ., space = "free_y", scales = "free_y") +
    scale_x_continuous(expand = expansion(mult = c(0.04, 0.04))) +
    labs(x = 'Overall Human Freedom', y = '', caption = '') +
    ggplot_clean_theme

# Right side (suicides)
gg_suicides <- free_suic_juxta_data %>%
    ggplot(aes(x = total_suicides_p100k, y = reorder(country, hf_score),
               group = prox_grp, color = color_grp)) +
    geom_path(color = "gray60") +
    geom_point(size = 3.5) +
    geom_point(color = "white", size = 1) +
...
grid_title_2 <- "A Comparison of the 25 Smallest Countries' Freedom Scores and Suicide Rates (2015)"
grid.arrange(gg_freedom_2, gg_suicides, ncol = 2, top = textGrob(grid_title_2))

freedom_suicide_juxta_plot <- arrangeGrob(gg_freedom_2, gg_suicides,
                                          ncol = 2, top = textGrob(grid_title_2))
ggsave(plot = freedom_suicide_juxta_plot, "../graphics/juxta_row_freedom_suicides.png",
       width = 7, height = 9, units = "in")
```
